...
RUN \
    mkdir pkg-config; \
    wget -qO- https://pkg-config.freedesktop.org/releases/pkg-config-$PKGCONFIG_VERSION.tar.gz | tar xvz -C pkg-config --strip-components=1; \
    cd pkg-config; \
...
RUN \
    mkdir sqlite3; \
    wget -qO- https://www.sqlite.org/2021/sqlite-autoconf-3370100.tar.gz | tar xvz -C sqlite3 --strip-components=1; cd sqlite3; \
...
RUN \
    mkdir nghttp2; \
    wget -qO- https://github.com/nghttp2/nghttp2/releases/download/v${NGHTTP2_VERSION}/nghttp2-${NGHTTP2_VERSION}.tar.gz | tar xvz -C nghttp2 --strip-components=1; cd nghttp2; \
...
RUN \
    mkdir openssl; \
    wget -qO- https://www.openssl.org/source/openssl-${OPENSSL_VERSION}.tar.gz | tar xvz -C openssl --strip-components=1; cd openssl; \
...
RUN \
    yum remove -y cmake; \
    mkdir cmake; \
    wget -qO- https://cmake.org/files/v3.22/cmake-3.22.1.tar.gz | tar xvz -C cmake --strip-components=1; cd cmake; \
    ./bootstrap; \
    make; \
    make -j ${NPROC} install; \
    cd ..; rm -rf cmake
...
RUN \
    mkdir geos; \
    wget -qO- http://download.osgeo.org/geos/geos-$GEOS_VERSION.tar.bz2 | tar xvj -C geos --strip-components=1; cd geos; \
    ./configure --prefix=$PREFIX CFLAGS="-O2 -Os"; \
    make -j ${NPROC} install; \
    cd ..; rm -rf geos
...
RUN \
    mkdir zstd; \
    wget -qO- https://github.com/facebook/zstd/archive/v${ZSTD_VERSION}.tar.gz | tar -xvz -C zstd --strip-components=1; cd zstd; \
...
RUN \
    mkdir geotiff; \
    wget -qO- https://download.osgeo.org/geotiff/libgeotiff/libgeotiff-$GEOTIFF_VERSION.tar.gz | tar xvz -C geotiff --strip-components=1; cd geotiff; \
    ./configure --prefix=${PREFIX} --with-proj=${PREFIX} --with-jpeg=${PREFIX} --with-zip=yes; \
    make -j ${NPROC} install; \
...
RUN \
    mkdir gdal; \
    wget -qO- http://download.osgeo.org/gdal/$GDAL_VERSION/gdal-$GDAL_VERSION.tar.gz | tar xvz -C gdal --strip-components=1; cd gdal; \
    ./configure \
        --disable-debug \
        --disable-static \
        --prefix=${PREFIX} \
